...
impl Updater {
    pub async fn check(&self) -> Result<Option<Update>> {
        ...
        // Preserve endpoint fallback validation before using the shared parser.
        remote_release.ok_or(Error::ReleaseNotFound)?;
        self.check_metadata(raw_json.ok_or(Error::ReleaseNotFound)?)
    }

    /// Checks already-fetched release metadata without making an HTTP request.
    ///
    /// The caller is responsible for fetching, bounding and authenticating the
    /// metadata before calling this method. Version comparison, platform target
    /// selection and package signature handling are the same as [`Self::check`].
    /// This method does not download or install the update.
    pub fn check_metadata(&self, raw_json: serde_json::Value) -> Result<Option<Update>> {
        let release = serde_json::from_value::<RemoteRelease>(raw_json.clone())?;
        let target = if let Some(target) = &self.target {
            target
        } else {
            updater_os().ok_or(Error::UnsupportedOs)?
